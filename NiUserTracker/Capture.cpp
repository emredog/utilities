...
#define START_CAPTURE_CHECK_RC(rc, what)												\
	...
	{																					\
        std::cout << "Failed to " << what << ": " << xnGetStatusString(rc) << std::endl;       \
		delete g_Capture.pRecorder;														\
		g_Capture.pRecorder = NULL;														\
		return false;																	\
	}

bool captureOpenWriteDevice(xn::Context* g_Context)
{
...
void captureBrowse(int a, xn::Context* g_Context)
{
#if (XN_PLATFORM == XN_PLATFORM_WIN32)
	OPENFILENAMEA ofn;
    ...
    captureOpenWriteDevice(g_Context);
}

void captureStart(int nDelay, xn::Context* g_Context)
{
	if (g_Capture.csFileName[0] == 0)
	{
        captureBrowse(0, g_Context);
	}
	...
	{
        if (!captureOpenWriteDevice(g_Context))
			return;
	}
...
void captureRestart(int, xn::Context* g_Context)
{
